...
public class ForumPresenterImpl extends BasePresenter<ForumFragment> implements ForumPresenter.Presenter {
    ...
    @Override
    public void fetchSign(String token, String formhash) {
        RetrofitHelper
                .getSignAPi()
                .fetchSign(token,formhash)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new BaseObserver<SignResult>() {
                    @Override
                    public void onSubscribe(Disposable d) {
                        super.onSubscribe(d);
                        addSubscribe(d);
                    ...
                    @Override
                    protected void onNextSuccess(SignResult scenicInfo) {
                        mView.showSign(scenicInfo);
                    }
                });
    }
}